    ...
    public class CvSparseMatIterator : DisposableCvObject, IEnumerable<CvSparseNode>
    {
        ...
        public CvSparseMat Mat
		{
            get
            {
                ...
                unsafe
                {
                    p = new IntPtr(((WCvSparseMatIterator*)_ptr)->mat);
                }
                if (p == IntPtr.Zero)
                    return null;
                else
		...
		public CvSparseNode Node
		{
            get
            {
                ...
                unsafe
                {
                    p = new IntPtr(((WCvSparseMatIterator*)_ptr)->node);
                }
                if (p == IntPtr.Zero)
                    return null;
                else
		...
		public int CurIdx
		{
            get
            {
                unsafe
                {
                    return ((WCvSparseMatIterator*)_ptr)->curidx;
                }
            }
		}
        #endregion
...
#endif
        public CvSparseNode Init(CvSparseMat mat)
        {
            if (mat == null)
            {
            ...
            this._mat = mat;
            IntPtr result = CvInvoke.cvInitSparseMatIterator(mat.CvPtr, this.CvPtr);
