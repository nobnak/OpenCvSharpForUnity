...
namespace OpenCvSharp
{
    ...
    public class CvMat : CvArr, ICloneable, IEnumerable<CvScalar>
    {
        ...
        {
            _ptr = CvInvoke.cvCreateMat(rows, cols, type);
            if (_ptr == IntPtr.Zero)
        ...
        public CvMat(int rows, int cols, MatrixType type, Array elements, bool copyData)
            //: this(rows, cols, type)
        {
            _ptr = CvInvoke.cvCreateMatHeader(rows, cols, type);
            if (_ptr == IntPtr.Zero)
                throw new OpenCvSharpException("Failed to create CvMat");
            ...
            CvInvoke.cvSetData(_ptr, gch.AddrOfPinnedObject(), Cv.AUTOSTEP);
            NotifyMemoryPressure(SizeOf);
        }
#if LANG_JP
        /// <summary>
            ...
            : this(rows, cols, type)
        {
            ...
            CvInvoke.cvSetData(_ptr, data, Cv.AUTOSTEP);
        }
#if LANG_JP
        /// <summary>
        ...
        public CvMat(string filename, LoadMode flags)
        {
            ...
            _ptr = CvInvoke.cvLoadImageM(filename, flags);
            if (_ptr == IntPtr.Zero)
            {
                throw new OpenCvSharpException("Failed to create CvMat");
            }
...
#endif
        public CvMat(IntPtr ptr, bool isEnabledDispose)
        ...
        public IntPtr Data
        {
            get
            {
                unsafe
                {
                    return new IntPtr(((WCvMat*)_ptr)->data);
                }
            }
            set
            {
                unsafe
                {
                    ((WCvMat*)_ptr)->data = value.ToPointer();
                }
            }
        }
#if LANG_JP
        ...
        public unsafe double* DataDouble
        {
            get
            {
                unsafe
                {
                    return (double*)(((WCvMat*)_ptr)->data);
                }
            }
            set
            {
        ...
        public PointerAccessor1D_Int16 DataArrayInt16
        {
            get
            {
                if (_dataArrayInt16 == null)
                {
                    unsafe
                    {
                        short* p = (short*)(((WCvMat*)_ptr)->data);
        ...
        public PointerAccessor1D_Single DataArraySingle
        {
            get
            {
                if (_dataArraySingle == null)
                {
                    unsafe
                    {
                        float* p = (float*)(((WCvMat*)_ptr)->data);
        ...
        public PointerAccessor1D_Double DataArrayDouble
        {
            get
            {
                if (_dataArrayDouble == null)
                {
                    unsafe
                    {
                        double* p = (double*)(((WCvMat*)_ptr)->data);
        ...
        public IntPtr RefCount
        {
            get
            {
                unsafe
                {
                    return new IntPtr(((WCvMat*)_ptr)->refcount);
                }
            }
            internal set
            {
                unsafe
                {
                    ((WCvMat*)_ptr)->refcount =(int*)value;
                }
            }
        }
#if LANG_JP
...
#endif
        public void CalibrationMatrixValues(CvSize image_size)
        {
            Cv.CalibrationMatrixValues(this, image_size);
        }
#if LANG_JP
        /// <summary>
        /// 様々なカメラ特性を計算する
...
#endif
        public void CalibrationMatrixValues(CvSize image_size, double aperture_width, double aperture_height)
        {
            Cv.CalibrationMatrixValues(this, image_size, aperture_width, aperture_height);
        }
#if LANG_JP
        /// <summary>
        /// 様々なカメラ特性を計算する
        ...
        public void CalibrationMatrixValues(
            CvSize image_size, double aperture_width, double aperture_height,
            out double fovx, out double fovy)
        {
            Cv.CalibrationMatrixValues(this, image_size, aperture_width, aperture_height, out fovx, out fovy);
        }
#if LANG_JP
        /// <summary>
        /// 様々なカメラ特性を計算する
...
#endif
        public void ComputeCorrespondEpilines(int which_image, CvMat fundamental_matrix, out CvMat correspondent_lines)
        {
            Cv.ComputeCorrespondEpilines(this, which_image, fundamental_matrix, out correspondent_lines);
        }
        #endregion
...
#endif
        public void UndistortPoints(CvMat dst, CvMat camera_matrix, CvMat dist_coeffs, CvMat R, CvMat P)
        {
            Cv.UndistortPoints(this, dst, camera_matrix, dist_coeffs, R, P);
        }
        #endregion        
        ...
        public CvScalar[] ToArray()
        {
            ...
            for (int r = 0; r < rows; r++)
            {
                ...
                {
                    result[r * cols + c] = CvInvoke.cvGet2D(ptr, r, c);
                }
            }
        ...
        public CvScalar[,] ToRectangularArray()
        {
            ...
            for (int r = 0; r < rows; r++)
            {
                for (int c = 0; c < cols; c++)
                {
                    result[r, c] = CvInvoke.cvGet2D(ptr, r, c);
                }
            }  
        ...
        public IEnumerator<CvScalar> GetEnumerator()
        {
            ...
            for (int r = 0; r < rows; r++)
            {
                for (int c = 0; c < cols; c++)
                {
                    yield return CvInvoke.cvGet2D(_ptr, r, c);
                }
            }
        }
        /// <summary>
