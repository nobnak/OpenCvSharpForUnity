...
namespace OpenCvSharp
{
    ...
    abstract public partial class CvArr : DisposableCvObject
    {
        ...
        {
            using (CvMemStorage line_storage = new CvMemStorage())
            {
                IntPtr result = CvInvoke.cvHoughLines2(this.CvPtr, line_storage.CvPtr, HoughLinesMethod.Standard, rho, theta, threshold, param1, param2);
        ...
        public CvLineSegmentPoint[] HoughLinesProbabilistic(double rho, double theta, int threshold, double param1, double param2)
        {
            using (CvMemStorage line_storage = new CvMemStorage())
            {
                IntPtr result = CvInvoke.cvHoughLines2(this.CvPtr, line_storage.CvPtr, HoughLinesMethod.Probabilistic, rho, theta, threshold, param1, param2);
                if (result == IntPtr.Zero)
                {
                    throw new OpenCvSharpException();
                }
...
#endif
        public void RandArr(CvRNG rng, DistributionType dist_type, CvScalar param1, CvScalar param2)
        {
            Cv.RandArr(rng, this, dist_type, param1, param2);
        }
        #endregion
        #region SaveImage
#if LANG_JP
