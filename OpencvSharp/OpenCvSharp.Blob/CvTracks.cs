...
namespace OpenCvSharp.Blob
{
    ...
    /// List of tracks.
    /// </summary>
    public class CvTracks : DisposableCvObject, IDictionary<CvID, CvTrack>
    {
        /// <summary>
        /// sizeof(CvTracks)
        /// </summary>
        ...
        public CvTracks()
        {
            _ptr = CvBlobInvoke.CvTracks_construct();
        ...
        protected override void Dispose(bool disposing)
        {
            if (!this.disposed)
            {
                ...
                try
                {
                    ...
                    if (IsEnabledDispose)
                    {
                        CvBlobInvoke.CvTracks_cvReleaseTracks(_ptr);
                        CvBlobInvoke.CvTracks_destruct(_ptr);
                    }
                    this.disposed = true;
                }
                finally
...
#endif
        public virtual void Add(CvID key, CvTrack value)
        {
            if (value == null)
            {
                throw new ArgumentNullException("value");
            }
            CvBlobInvoke.CvTracks_Add(_ptr, key, value.CvPtr);
        }
#if LANG_JP
        /// <summary>
...
#endif
        public virtual bool ContainsKey(CvID key)
        {
            return CvBlobInvoke.CvTracks_ContainsKey(_ptr, key);
        }
#if LANG_JP
		/// <summary>
...
#endif
        public virtual ICollection<CvID> Keys
        {
            get
            {
                CvID[] keys = new CvID[Count];
                CvBlobInvoke.CvTracks_Keys(_ptr, keys);
                return keys;
            }
        }
#if LANG_JP
...
#endif
        public virtual bool Remove(CvID key)
        {
            if (ContainsKey(key))
            {
                CvTrack track = this[key];
                // mapからの削除
                bool result = CvBlobInvoke.CvTracks_RemoveAt(_ptr, key);
...
#endif
        public virtual bool TryGetValue(CvID key, out CvTrack value)
        {
            IntPtr valuePtr;
            if (CvBlobInvoke.CvTracks_TryGetValue(_ptr, key, out valuePtr))
            {
        ...
        public virtual ICollection<CvTrack> Values
        {
            get
            {
                ...
                CvBlobInvoke.CvTracks_Values(_ptr, values);
...
#endif
        public virtual CvTrack this[CvID key]
        {
            get
            {
                IntPtr p = CvBlobInvoke.CvTracks_get(_ptr, key);
                if (p == IntPtr.Zero)
                    return null;
                else
            ...
            set
            {
                ...
                CvBlobInvoke.CvTracks_set(_ptr, key, v);
            }
        }
        #endregion
...
#endif
        public virtual void Add(KeyValuePair<CvID, CvTrack> item)
        {
            IntPtr value = (item.Value == null) ? IntPtr.Zero : item.Value.CvPtr;
            CvBlobInvoke.CvTracks_Add(_ptr, item.Key, value);
        }
#if LANG_JP
		/// <summary>
...
#endif
        public virtual bool Contains(KeyValuePair<CvID, CvTrack> item)
        {
            IntPtr value = (item.Value == null) ? IntPtr.Zero : item.Value.CvPtr;
            return CvBlobInvoke.CvTracks_Contains(_ptr, item.Key, value);
        }
#if LANG_JP
        /// <summary>
...
#endif
        public virtual void CopyTo(KeyValuePair<CvID, CvTrack>[] ary, int aryIndex)
        {
            if (ary == null)
            ...
            int i = 0;
            foreach (KeyValuePair<CvID, CvTrack> item in this)
            {
                ary[i + aryIndex] = item;
                i++;
            }
        ...
        public virtual int Count
        {
            get
            {
                return CvBlobInvoke.CvTracks_Count(_ptr);
            }
        }
#if LANG_JP
		/// <summary>
...
#endif
        public virtual bool Remove(KeyValuePair<CvID, CvTrack> item)
        {
            if (item.Value == null)
                throw new ArgumentNullException("item.Value");

            bool result = CvBlobInvoke.CvTracks_Remove(_ptr, item.Key, item.Value.CvPtr);
        ...
        /// </summary>
#endif
        public IEnumerator<KeyValuePair<CvID, CvTrack>> GetEnumerator()
        {
            int count = Count;
            CvID[] keys = new CvID[count];
            IntPtr[] values = new IntPtr[count];
            CvBlobInvoke.CvTracks_GetKeysAndValues(_ptr, keys, values);

            for (int i = 0; i < count; i++)
            {
                CvTrack v = (values[i] == IntPtr.Zero) ? null : new CvTrack(values[i]);
                yield return new KeyValuePair<CvID, CvTrack>(keys[i], v);
            }
        }
#if LANG_JP
		/// <summary>
