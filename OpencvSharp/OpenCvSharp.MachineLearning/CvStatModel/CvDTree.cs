...
namespace OpenCvSharp.MachineLearning
{
	...
	public class CvDTree : CvStatModel
    {
        ...
        protected override void Dispose(bool disposing)
        {
            if (!this.disposed)
            {
                ...
                try
                {
                    ...
                    if (IsEnabledDispose)
                    {
				        MLInvoke.CvDTree_destruct(_ptr);
		...
		public virtual bool Train(CvDTreeTrainData trainData, CvMat subsampleIdx)
        {
			...
			return MLInvoke.CvDTree_train(_ptr, trainData.CvPtr,  subsampleIdxPtr);
        }
		#endregion
		...
		public virtual CvDTreeNode Predict(CvMat sample, CvMat missingDataMask, bool preprocessedInput)
		{
			...
			IntPtr result = MLInvoke.CvDTree_predict(_ptr, sample.CvPtr, missingDataMaskPtr, preprocessedInput);
		...
		{
			IntPtr p = MLInvoke.CvDTree_get_var_importance(_ptr);
			if(p == IntPtr.Zero)
				return null;
			else
		...
		{
			IntPtr p = MLInvoke.CvDTree_get_root(_ptr);
			if(p == IntPtr.Zero)
				return null;
			else
		...
		{
			return MLInvoke.CvDTree_get_pruned_tree_idx(_ptr);
		}
		...
		{
			IntPtr p = MLInvoke.CvDTree_get_data(_ptr);
			if(p == IntPtr.Zero)
				return null;
			else
		...
		public override void Read(CvFileStorage fs, CvFileNode node)
        {
			...
			MLInvoke.CvDTree_read(_ptr, fs.CvPtr, node.CvPtr);
        }
#if LANG_JP
        /// <summary>
        /// ファイルストレージからモデルを読み込む
		...
		public virtual void Read(CvFileStorage fs, CvFileNode node, CvDTreeTrainData data)
        {
			...
			MLInvoke.CvDTree_read(_ptr, fs.CvPtr, node.CvPtr, data.CvPtr);
        }
		...
		public virtual void Write(CvFileStorage fs)
        {
            if (fs == null)
                throw new ArgumentNullException("fs");
            MLInvoke.CvDTree_write(_ptr, fs.CvPtr);
        }
#if LANG_JP
        /// <summary>
        /// モデルをファイルに書き込む
		...
		public override void Write(CvFileStorage fs, string name)
        {
            ...
            MLInvoke.CvDTree_write(_ptr, fs.CvPtr, name);
        }
