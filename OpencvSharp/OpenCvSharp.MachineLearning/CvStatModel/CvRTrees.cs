...
namespace OpenCvSharp.MachineLearning
{
	...
	public class CvRTrees : CvStatModel
	{
            ...
            //: this(ML.CvRTrees_construct())
        {
            ...
            MLInvoke.CvRTrees_construct(_ptr);
        }
#if LANG_JP
        /// <summary>
        /// ポインタから初期化
        ...
        public virtual double Predict(CvMat sample, CvMat missing)
		{
            ...
            return MLInvoke.CvRTrees_predict(_ptr, sample.CvPtr, missingPtr);
		}
		#endregion
		...
		{
			IntPtr p = MLInvoke.CvRTrees_get_var_importance(_ptr);
			if(p == IntPtr.Zero)
				return null;
			else
        ...
        public virtual float GetProximity(CvMat sample1, CvMat sample2, CvMat missing1, CvMat missing2)
        {
            ...
            return MLInvoke.CvRTrees_get_proximity(_ptr, sample1.CvPtr, sample2.CvPtr, missing1Ptr, missing2Ptr);
        }
		...
		{
			IntPtr p = MLInvoke.CvRTrees_get_active_var_mask(_ptr);
			if(p == IntPtr.Zero)
				return null;
			else
				return new CvMat(p, false);
		...
		public CvRNG GetRng()
		{
			IntPtr p = MLInvoke.CvRTrees_get_rng(_ptr);
			if(p == IntPtr.Zero)
				return null;
			else
		...
		{
			return MLInvoke.CvRTrees_get_tree_count(_ptr);
		}
        ...
        {
            IntPtr p = MLInvoke.CvRTrees_get_tree(_ptr, i);
            if (p == IntPtr.Zero)
                return null;
            else
		...
		public override void Write(CvFileStorage storage, String name) 
        {
            ...
            MLInvoke.CvRTrees_write(_ptr, storage.CvPtr, name);
        }
		...
		public override void Read(CvFileStorage fs, CvFileNode node) 
        {
            ...
            MLInvoke.CvRTrees_read(_ptr, fs.CvPtr, node.CvPtr);
        }
		#endregion
        #endregion
	}
