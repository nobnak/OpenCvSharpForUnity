...
#pragma warning restore 1591
		...
		public CvBoost()
		{
			_ptr = MLInvoke.CvBoost_construct_default();
		...
		{    
			...
			_ptr = MLInvoke.CvBoost_construct_training(
				trainData.CvPtr, 
				(int)tflag,
				responses.CvPtr,
				varIdxPtr, 
        ...
        protected override void Dispose(bool disposing)
        {
            if (!disposed)
            {
                ...
                try
                {
                    ...
                    if (IsEnabledDispose)
                    {
                        MLInvoke.CvBoost_destruct(_ptr);
		...
		{    
			MLInvoke.CvBoost_prune(_ptr, slice);
		}
		...
		{
            IntPtr p = MLInvoke.CvBoost_get_weights(_ptr);
            if(p == IntPtr.Zero)
                return null;
            else
		...
		{
            IntPtr p = MLInvoke.CvBoost_get_subtree_weights(_ptr);
            if(p == IntPtr.Zero)
                return null;
            else
			    return new CvMat(p, false);
		...
		{
            IntPtr p = MLInvoke.CvBoost_get_weak_response(_ptr);
            if(p == IntPtr.Zero)
                return null;
            else
			    return new CvMat(p, false);
		...
		{
			IntPtr p = MLInvoke.CvBoost_get_params(_ptr);
            if(p == IntPtr.Zero)
                return null;
            else
        ...
        {
            IntPtr p = MLInvoke.CvBoost_get_weak_predictors(_ptr);
            if (p == IntPtr.Zero)
                return null;
            else
                return new CvSeq(p);
		...
		public override void Write(CvFileStorage storage, string name) 
        {
            ...
            MLInvoke.CvBoost_write(_ptr, storage.CvPtr, name);
        }
        ...
        public override void Read(CvFileStorage storage, CvFileNode node) 
        {
            ...
            MLInvoke.CvBoost_read(_ptr, storage.CvPtr, node.CvPtr);
        }
		#endregion
        #endregion
	}
